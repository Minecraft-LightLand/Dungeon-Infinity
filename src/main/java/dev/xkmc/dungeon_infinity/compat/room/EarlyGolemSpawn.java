...
public class EarlyGolemSpawn extends AbstractGolemSpawn {
	...
	public static void add(ConfigDataProvider.Collector map) {
		...
		// trial
		{
			...
			// BASIC: 基础混编
			map.add(GolemDungeons.TRIAL, GolemSpawnData.EARLY_ROOM_BASIC, new TrialConfig().setReward(MazeRoomLootGen.STONE_ROOM_BASIC)
			...
			// RANGED: 远程火力网+火箭兵
			map.add(GolemDungeons.TRIAL, GolemSpawnData.EARLY_ROOM_RANGED, new TrialConfig().setReward(MazeRoomLootGen.STONE_ROOM_RANGED)
					.add(of(HUMANOID_RANGED, 2))
					.add(of(HUMANOID_MELEE, 1), of(HUMANOID_RANGED, 2))
			);

			// RIDER: 火箭骑兵+药水兵
			map.add(GolemDungeons.TRIAL, GolemSpawnData.EARLY_ROOM_RIDER, new TrialConfig().setReward(MazeRoomLootGen.STONE_ROOM_RIDER)
					.add(of(HUMANOID_RIDER_MELEE, 1))
					.add(of(HUMANOID_RIDER_MELEE, 1), of(HUMANOID_RIDER_RANGED, 1))
			);

			// MIXED: 全兵种混合
			map.add(GolemDungeons.TRIAL, GolemSpawnData.EARLY_ROOM_MIXED, new TrialConfig().setReward(MazeRoomLootGen.STONE_ROOM_MIXED)
					.add(of(HUMANOID_MELEE, 1), of(HUMANOID_RANGED, 1))
					.add(of(HUMANOID_RIDER_MELEE, 1), of(HUMANOID_RANGED, 1))
					.add(of(LARGE_1, 1), of(HUMANOID_RANGED, 1))
			);

			map.add(GolemDungeons.TRIAL, GolemSpawnData.EARLY_QUAD, new TrialConfig().setReward(MazeRoomLootGen.STONE_QUAD)
